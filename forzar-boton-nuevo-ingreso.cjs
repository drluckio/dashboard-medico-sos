...
const backupPath = path.join(
  __dirname,
  "src",
  `ConsentFormsModule_backup_force_new_hire_${Date.now()}.jsx`
);
...
function applyReplace(search, replacement, label) {
...
/*
  1. Agregar Nuevo ingreso al selector de consentimientos.
*/
if (!content.includes('id: "new_hire"')) {
  applyRegex(
    /const consentFormOptions = \[\s*/,
    'const consentFormOptions = [\n  { id: "new_hire", label: "Nuevo ingreso" },\n',
    "opción Nuevo ingreso en consentFormOptions"
  );
}

/*
  2. Agregar Nuevo ingreso a selección por defecto.
*/
if (!content.includes('"new_hire",') && content.includes("selectedFormTypes")) {
  applyRegex(
    /const \[selectedFormTypes, setSelectedFormTypes\] = useState\(\[\s*/,
    'const [selectedFormTypes, setSelectedFormTypes] = useState([\n    "new_hire",\n',
    "Nuevo ingreso seleccionado por defecto"
  );
}

/*
  3. Función imprimible de nuevo ingreso.
*/
  ...
  return \`
    <main class="page">
      ...
      <ul class="checklist">
        ...
        <li><span class="check"></span>Entiendo que debo informar molestias, antecedentes, enfermedades, tratamientos o limitaciones relevantes.</li>
...
if (!content.includes("function buildNewHireConsent")) {
  applyReplace(
    ...
    newHireFunction + "function buildAlcoholConsent(form, companies, plants) {",
    "función buildNewHireConsent"
  );
}

/*
  4. Agregar builder new_hire.
*/
...
/*
  5. Insertar botón visible antes del botón Alcoholimetría.
*/
if (!content.includes('printDocument("new_hire", form, companies, plants)')) {
  applyRegex(
    /(<button\s+type="button"\s+onClick=\{\(\) => printDocument\("alcohol", form, companies, plants\)\})/,
    `<button
              type="button"
...
/*
  6. Ajustar grid de botones rápidos si sigue en 4 columnas.
*/
...
console.log("Opción new_hire:", content.includes('id: "new_hire"') ? "OK" : "NO");
console.log("Seleccionado por defecto:", content.includes('"new_hire"') ? "OK" : "NO");
console.log("Función buildNewHireConsent:", content.includes("function buildNewHireConsent") ? "OK" : "NO");
console.log("Builder:", content.includes("new_hire: buildNewHireConsent") ? "OK" : "NO");
console.log("Botón:", content.includes('printDocument("new_hire", form, companies, plants)') ? "OK" : "NO");
