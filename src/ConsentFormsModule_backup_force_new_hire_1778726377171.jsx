...
function buildSelectedPages(types, form, companies, plants) {
  ...
  return selectedTypes
    ...
    .filter(Boolean)
    .map((builder) => builder(form, companies, plants))
    .join("");
}

function buildDocument(types, form, companies, plants) {
  const pages = buildSelectedPages(types, form, companies, plants);
...
function printDocument(types, form, companies, plants) {
  const selectedTypes = Array.isArray(types) ? types : [types];
  ...
  printWindow.document.write(buildDocument(selectedTypes, form, companies, plants));
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Primero llena los datos generales. Después selecciona qué consentimientos
          necesitas imprimir: alcoholimetría, antidoping, revisión médica, referencia
          médica o cualquier combinación.
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-zinc-500">
          Selecciona uno o varios formatos según se necesite.
        </p>
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-3">
          <button
            type="button"
            onClick={() => printDocument(selectedFormTypes, form, companies, plants)}
